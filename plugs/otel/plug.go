...
func (p *Plug) Gen(option gen.Option, implGoTypeMetes []gen.InterfaceGoTypeMeta) (res []gen.GenResult, err error) {
	...
	for _, v := range implGoTypeMetes {
		...
		if err != nil {
			...
			return nil, common.ParseError("failed to parse interface for otel plugin").
				WithCause(err).
				WithPlugin("@otel").
				WithInterface(v.Name).
				WithDetails("unable to parse interface metadata for OpenTelemetry instrumentation").
				Build()
		}
